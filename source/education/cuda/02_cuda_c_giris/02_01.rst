...
Programlama Dilleri
^^^^^^^^^^^^^^^^^^^
* Programlama dilleri sunduğu performans ve esneklik açısından GPU  programlamada tercih edilen bir başka yöntemdir. **Örnek** : CUDA
...
İlerleyen bölümlerde genel bir CUDA programında sıklıkla kullandığımız fonksiyonlardan ve program akışından bahsediyor olacağız ancak kullanacağımız terimler ve genel anlamda CUDA hakkında biraz daha bilgi sahibi olmak için aşağıdaki görseli inceleyelim.
