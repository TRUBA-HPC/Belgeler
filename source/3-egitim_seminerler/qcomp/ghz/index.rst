...
Bu doküman, TRUBA sisteminde NVIDIA cuQuantum, Qiskit Aer GPU backend, MPI ve Slurm kullanılarak GHZ benchmark çalıştırılması için temel adımları açıklamaktadır.
...
* Apptainer tabanlı konteyner kullanımı bilinmelidir.
...
.. code-block:: bash
    ...
    mkdir -p ~/qcomp/ghz/{jobs,logs,containers}
    cd ~/qcomp/ghz
...
TRUBA üzerinde NVIDIA cuQuantum ortamı Apptainer imajı olarak kullanılabilir. Eğer hazır bir ``.sif`` imajı mevcut ise doğrudan bu imaj kullanılabilir.
...
Hazır imaj bulunmuyorsa, kurum politikalarına uygun olmak kaydıyla NGC/Docker tabanlı cuQuantum imajı Apptainer formatına dönüştürülebilir.
...
.. note::
    ...
    İmaj etiketi kurumun izin verdiği sürüm ve CUDA uyumluluğuna göre uyarlanmalıdır.

GPU Erişiminin Doğrulanması
===========================

GHZ benchmark çalıştırılmadan önce konteynerin GPU cihazlarını görebildiği doğrulanmalıdır.
...
GHZ Doğrulama Betiği
====================

Aşağıdaki Python betiği, MPI rank bilgilerini okuyarak rank-GPU eşleşmesini yazdırır, Qiskit Aer GPU backend ile GHZ durumu oluşturur ve fidelity değerini raporlar.
...
Aşağıdaki örnek, 2 node / 8 rank / node başına 4 GPU kullanarak GHZ betiğini çalıştırır.
...
İş kuyruğa aşağıdaki şekilde gönderilebilir.
...
* Çoklu node koşularda MPI başlatıcısı olarak doğrudan Slurm'un ``srun --mpi=pmix`` çağrısı kullanılmalıdır.
...
* H100/H200 türündeki farklı node'lar üzerinde çalışılabileceği için kullanıcılar gerekli gördüğünde Slurm constraint eklemelidir.
